...
<head>
    ...
    <title>{{ $title ?? 'Compliance Calendar' }}</title>
    <meta name="description" content="Month-by-month due dates for PF, ESI, TDS, GST, PT and Form 16.">
    <style>
...
<x-crmleaf::compliance-calendar
    ...
    :result="$result ?? null"
    :error="$error ?? null"
    :heading="$title ?? 'Compliance Calendar'"
/>
...
<footer class="crmleaf-tool__colophon">
    ...
    <p>
        Computed by <a href="https://github.com/crmleaf/compliance-calendar">crmleaf/compliance-calendar</a>,
        MIT licensed. A calculation library, not tax advice - verify against your own
        compliance obligations before filing.
    </p>
</footer>

@if (config('compliance-calendar.assets.script', false))
    <script src="{{ asset('vendor/compliance-calendar/compliance-calendar.js') }}" defer></script>
@endif
