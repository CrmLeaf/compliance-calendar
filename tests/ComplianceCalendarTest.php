...
namespace Crmleaf\Payroll\Tools\ComplianceCalendar\Tests;

use Crmleaf\Payroll\Calculators\ComplianceCalendar;
use Crmleaf\Payroll\Tools\ComplianceCalendar\ComplianceCalendarServiceProvider;
 ...
 */
final class ComplianceCalendarTest extends TestCase
{
    /**
     * Held as a literal rather than as ComplianceCalendar::class so that a calculator
     * which does not exist fails as one readable assertion instead of a fatal
     * error that takes the rest of the file with it.
     */
    private const CALCULATOR = 'Crmleaf\Payroll\Calculators\ComplianceCalendar';

    private const METHOD = 'forFinancialYear';
    ...
    {
        return [ComplianceCalendarServiceProvider::class];
    }
    ...
    protected function defineEnvironment($app): void
    {
        ...
        // pins down.
        $app['config']->set('compliance-calendar.route.enabled', true);
     ...
     * `$this->calculator->forFinancialYear(...$request->payload())`.
     *
    ...
    {
        $calculator = $this->app->make(ComplianceCalendar::class);

        self::assertInstanceOf(ComplianceCalendar::class, $calculator);
        self::assertSame($calculator, $this->app->make(ComplianceCalendar::class), 'The binding should be a singleton.');
    }

    public function test_the_configuration_is_merged(): void
    {
        self::assertSame('compliance-calendar', $this->app['config']->get('compliance-calendar.route.name'));
        self::assertSame('Compliance Calendar', $this->app['config']->get('compliance-calendar.view.title'));
    }

    public function test_the_route_is_off_until_it_is_asked_for(): void
    {
        /** @var array{route: array{enabled: bool}} $shipped */
        $shipped = require __DIR__.'/../config/compliance-calendar.php';
    ...
    {
        $html = Blade::render('<x-crmleaf::compliance-calendar />');

        self::assertStringContainsString('data-crmleaf-tool="compliance-calendar"', $html);
    ...
    {
        $response = $this->postJson('/tools/compliance-calendar', [
        ...
        $response->assertJsonPath('tool', 'compliance-calendar');
    }

    public function test_incomplete_input_is_rejected_rather_than_guessed_at(): void
    {
        $this->postJson('/tools/compliance-calendar', [])
            ->assertStatus(422)
